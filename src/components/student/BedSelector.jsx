...
const STATUS_CONFIG = {
  ...
  BOOKED: { label: 'Booked', bg: 'bg-red-50', border: 'border-red-400', text: 'text-red-700', badge: 'bg-red-100 text-red-700', selectable: false },
  blocked: { label: 'Blocked', bg: 'bg-gray-50', border: 'border-gray-300', text: 'text-gray-500', badge: 'bg-gray-100 text-gray-500', selectable: false },
  BLOCKED: { label: 'Blocked', bg: 'bg-gray-50', border: 'border-gray-300', text: 'text-gray-500', badge: 'bg-gray-100 text-gray-500', selectable: false },
};
  ...
  if (!roomId) {
    return (
      <div className="space-y-3">
        <h2 className="text-lg font-semibold text-gray-800">Select a Bed</h2>
        <p className="text-sm text-gray-500">Please select a room first</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-2 text-sm text-gray-500">Loading beds...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 text-sm">No beds found in this room</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <button
        ...
        className={`
          ...
          ${isSelectable && isSelected ? 'border-indigo-500 bg-indigo-50 ring-4 ring-indigo-100 shadow-md' : ''}
          ${isSelectable && !isSelected ? 'border-emerald-300 bg-white hover:border-emerald-400 hover:shadow-sm cursor-pointer' : ''}
        `}
      >
        <div className="flex items-center gap-1 mb-2">
          {position === 'UP' && <ArrowUp className="w-4 h-4 text-indigo-500" />}
          {position === 'DOWN' && <ArrowDown className="w-4 h-4 text-indigo-500" />}
          <span className="text-xs font-medium text-gray-500 uppercase">{position || 'N/A'}</span>
        </div>
        <Bed
          className={`w-8 h-8 mb-2 ${
            isSelected ? 'text-indigo-600' : isSelectable ? 'text-emerald-600' : config.text
          }`}
          strokeWidth={1.5}
        />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        <span className="flex items-center gap-1 text-xs">
          <span className="w-3 h-3 rounded-full bg-gray-300" /> Blocked
        </span>
      </div>
